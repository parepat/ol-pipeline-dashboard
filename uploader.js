...
function build(html){
  ...
  # เข้ารหัส renderCode เป็น base64 แล้วให้ browser decode
  # วิธีนี้ไม่มี HTML ใดๆ ใน script content
  render_str = '\n'.join(renderCode)
  import base64
  b64 = base64.b64encode(render_str.encode('utf-8')).decode('ascii')

  # สร้าง script ที่ decode แล้ว eval — ปลอดภัย 100%
  decode_script = (
    '<script>'
    'var _b="' + b64 + '";'
    'var _s=atob(_b);'
  ...
  if body_end >= 0:
    html = html[:body_end] + inject + html[body_end:]
  else:
    html += inject

  # 6. Download
  today = __import__('datetime').datetime.today()
  fname = 'OL_Dashboard_{:02d}{:02d}{}.html'.format(today.day, today.month, today.year)
